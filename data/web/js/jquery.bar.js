;(function($) {
	...
	$.show_notify_bar = function(options) {
		...
		if(!$('.jbar').length){
			...
			var _wrap_bar = $(document.createElement('div')).addClass('jbar').addClass(opts.class_name);
            // _wrap_bar.attr('id','jbar');
			if(opts.removebutton){
				var _remove_cross = $('<button type="button" class="close" aria-hidden="true">&times;</button>');
